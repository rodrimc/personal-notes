...
react instantaneously.  Moreover, as detailed in \Cref{sec:sem}, \Ceu\
...
The \Ceu\ program depicted in \Cref{lst:ceu-LEDs} illustrates these
...
Line~1 of \Cref{lst:ceu-LEDs} declares the external input event \ceulst{KEY}.
...
Thus the program of \Cref{lst:ceu-LEDs} finishes when a input event
...
by \Cref{lst:ceu-internal}.  The program of \Cref{lst:ceu-internal} consists
...
\Cref{lst:ceu-finalize} depicts a program containing a \ceulst{finalize}
statement.
...
In \Cref{lst:ceu-finalize}, the \ceulst{finalize} block~(lines~6--8)
executes when the first trail~(lines~1--9) finishes.  Thus the process of
sending the request in variable \ceulst{msg} is properly canceled (by
calling the \ceulst{\_send\_cancel} function) if the request takes more then
50ms to complete, i.e., the second trail finishes.
...
Functions~$\blocked$ and~$\clear$ are formally defined next.  In the
definitions, expressions of the form~$\|\alpha|$ denote the length of event
stack~$\alpha$ and the operator~$\cdot$ denotes multiplication on~$N$.
...
proof of \Cref{thm:sem:outerx:exhaust}, the main result of the report, which
will probably lead to the inclusion of new lemmas.

As potential future work we include: (i)~the definition of program
equivalence via operation~$\Router$ and the investigation of interesting
reduction procedures for programs; (ii)~the inclusion of the distinction
between internal and external events; (iii)~the removal of the restriction
that (internal) \ceu{\Await{e}} instructions cannot awake in the same
reaction they were executed, which is relatively new feature of the
language; and~(iv) the comparison of this semantics with that of other
...
\begin{proof}\label{proof:lem:sem:inneri:irr-unique}
  ...
  \item Again, for the sake of a contradiction, suppose there is are~$j<i$
    ...
    Then by \cref{lem:sem:inneri:irr-unique:1}, since~$i>j$, $\delta'$ could
    not exist, which is an absurd.  Therefore, our assumption that there is
