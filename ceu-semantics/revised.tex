...
\title{A Deterministic, Terminating and Memory Bounded Formal Semantics
  for the Synchronous Language \Ceu}
\begin{document}
\maketitle
...
\hfil\hbox{%
  \vtop{%
    ...
    \halign{\strut\hfil$#$&$\;#$\hfil&\qquad#\hfil\cr
      ...
      \mid& \MARK{\ceu{\Term}}         & final state\cr
    }%
  }%
}\hfil%
...
\begin{definition}[label={def:orig:blocked}]
  ...
  \todo{Intuitivamente, um programa~$p$ está bloqueado se toda trilha de~$p$
    ...
    está no topo da pilha ou que foi emitido na reação atual, ~(ii) acabou
    de emitir um evento ($\ceu{\protect\Emitting}$) e esse evento ainda não
    foi consumido (i.e., está na pilha),~\MARK{(iii) ou as trilhas estão
    paradas num $\ceu{\protect\Fin}$}.  É isso?}
  %%
  \begin{align*}
    \blocked(\ceu\Term,\alpha,n)
    &=1\\
...
\begin{lemma}[label={lem:orig:inner-step-or-blocked}]
  ...
  if~$p\ne\ceu{\Break}$\todo{Então não é $\forall p\in{P}$}~then either
  \[
...
\begin{definition}[label={def:orig:halted}]
