...
\begin{definition}[label={def:orig:clear}]
  Function $\clear\colon{P}\to{P'}$, \MARK{where $P'=\{\ceu{\Attr{v}{a}}\}$}, 
...
\begin{definition}[label={def:orig:inner-step},name={Reaction inner-step}]
  ...
  \noindent\emph{Await and emit}
    ...
    \begin{alignat}{2}
      ...
      &\MARK{\<\ceu{\Skip\And{\;p}},\alpha,m>}
      \MARK{\step{n}\<p,\alpha,m>}
      &&
      \\[1\jot]
      %%
      \label{def:orig:inner:and-skip-right}
      &\MARK{\<\ceu{p\And\Skip},\alpha,m>}
      \MARK{\step{n}\<p,\alpha,m>}
      &&
      \\[1\jot]
      %%
    ...
    \begin{alignat}{2}
      ...
      &\MARK{\<\ceu{\Skip\Or{\;p}},\alpha,m>}
      \MARK{\step{n}\<p',\alpha,m>}
      &&\quad\parbox{11em}{\MARK{$p'=\clear(p)$}}
      \\[1\jot]
      %%
      \label{def:orig:inner:or-skip-right}
      &\MARK{\<\ceu{p\Or\Skip},\alpha,m>}
      \MARK{\step{n}\<p',\alpha,m>}
      &&\quad\parbox{11em}{\MARK{if~$\blocked(p,\alpha,n)=1$,\\
...
\begin{proof}
  ...
  \item$p=\ceu{p';p''}$, for some~$p'$, $p''\in{P}$.
    ...
    \item$p'=\ceu{\Break}$.  Then~$d_1$ and~$d_2$ are instances of
      ...
      $\alpha_1=\alpha_2=\alpha$ and~$m_1=m_2=m$.
    \item$p'\ne\ceu{\Skip,\Attr{v}{a}},\ceu{\Break}$.  Then~$d_1$ and~$d_2$ are
      instances of rule~\eqref{def:orig:inner:seq}.  Thus there are
  ...
  \item$p=\ceu{p'\Atloop{p''}}$, for some~$p'$, $p''\in{P}$.
    ...
    \item$p=\ceu{\Skip\Atloop{p'}}$, for some~$p'\in{P}$.  
      Then~$d_1$ and~$d_2$ are instances
      of axiom~\eqref{def:orig:inner:atloop-skip}, and as such,
    ...
    \item$p=\ceu{p'\Atloop{p''}}$, for some~$p'$, $p''\in{P}$ such
      ...
      are instances of rule~\eqref{def:orig:inner:atloop}.  Thus there are
      derivations~$d_1'$ and~$d_2'$ such that
      \[
        d_1'\Vdash\<p',\alpha,m>\step{n}\<p_1',\alpha_1,m_1>
        \quad\text{and}\quad
  ...
  \item\label{thm:orig:det-inner:and} $p=\ceu{p'\And{p''}}$, for some~$p'$,
    ...
    \item$p=\ceu{p'\And\Attr{v}{a}}$, for some~$v\in{V}$, $a\in{A}$
      and~$p'\in{P}$.  %Similar to \Cref{thm:orig:det-inner:and-attr-left}.
      \MARK{\\
      Then either~$\blocked(p')=0$ or~$\blocked(p')=1$.  If~$\blocked(p')=0$ 
      then this case becomes \Cref{thm:orig:det-inner:and-left}.  Otherwise,
...
\begin{lemma}[label={lem:orig:inner-step-or-blocked}]
  For all~$p\in{P}$, $e,\alpha\in{E^*}$, $m\in\mathcal{M}$, and~$n\in{N}$,
  if~$p\ne\ceu{\Skip},\ceu{\Attr{v}{a}},\ceu{\Break}$ then either
  \[
    \exists{\delta\in\Delta}(\<p,e\alpha,m>\step{n}\delta)
    \quad\text{or}\quad
    \blocked(p,e\alpha,n)=1,
  \]
  but not both.
\end{lemma}
\begin{proof}
  \MARK{By induction on the structure of programs.
    There are ten possibilities depending on the structure of~$p$.  
  }
  \begin{case}
  \item
  \MARK{ 
    $p=\ceu{\Await(e)}$, for some~$e\in{E}$.  
    Then we can derive~$\delta$ using the axiom~\eqref{def:orig:inner:await}, 
    and as such, $\delta=\<\ceu{\Awaiting(e,n')},ea,m>$, with $n'=n+1$, 
    and according to 
    definition~\ref{def:orig:blocked}, $\blocked(await(e'),e\alpha,n) = 0$.
  }
    %%
  \item $p=\ceu{\Awaiting(e,n')}$, for some~$e\in{E}$ and~$n'\in{N}$.

  \MARK{ 
    Aqui tive um problema: $\blocked(\ceu{\Awaiting(e',n')},e\alpha,n)=1$ se
    $n=n'$. Porém, o axioma~\eqref{def:orig:inner:awaiting} define uma regra 
    para a configuração $\<\ceu{\Awaiting(e,n')},e\alpha,m>$ se $n'\leq n$.
  }
    %%
  \item
  \MARK{
    $p=\ceu{\Emit(e)}$, for some~$e\in{E}$.  
    Then we can derive~$\delta$ using the axiom~\eqref{def:orig:inner:emit}, 
    and as such, $\delta=\<\ceu{\Emitting(n')},e\alpha,m>$ with~$n'=\|\alpha|$,
    and according to 
    definition~\ref{def:orig:blocked}, $\blocked(\ceu{\Emit(e')},e\alpha,n)=0$.
  }
    %%
  \item
  \MARK{ 
    $p=\ceu{\Emitting(e,n')}$, for some~$e\in{E}$ and~$n'\in{N}$.
  \begin{case}
    \item 
      For $n'=\|e\alpha|$. Then we can derive~$\delta$ using the 
      axiom~\eqref{def:orig:inner:emitting}, and as such, 
      $\delta=\<\ceu\Skip,e\alpha,m>$, and according to 
      definition~\ref{def:orig:blocked}, 
      $\blocked(\ceu{\Emitting(e,n')},e\alpha,n)=0$.
    \item 
      For $n'\ne \|e\alpha|$. Then there is no rule we can use to 
      derive~$\delta$ and according to 
      definition~\ref{def:orig:blocked}, 
      $\blocked(\ceu{\Emitting(e,n')},e\alpha,n)=1$.
  \end{case}
  }
    %%
  \item
  \MARK{
    $p=\ceu{\Ifelse{b}{p'}{p''}}$, for some~$b\in{B}$ and~$p'$,
    $p''\in{P}$.
    According to definition~\ref{def:orig:blocked},
    $\blocked(\ceu{\Ifelse{b}{p'}{p''}},e\alpha,n)=0$
    \begin{case}
    \item$\eval(b,m)=1$.  Then we can derive~$\delta$ using the
      axiom~\eqref{def:orig:inner:if-true}, and as such, 
      $\delta=\<p',e\alpha,m>$.
    \item$\eval(b,m)=0$.  Then we can derive~$\delta$ using the
      axiom~\eqref{def:orig:inner:if-false}, and as such, 
      $\delta=\<p'',e\alpha,m>$.
    \end{case}
  }
  \item
  \MARK{ 
    $p=\ceu{p';p''}$, for some~$p'$, $p''\in{P}$.
    According to definition~\ref{def:orig:blocked},
    $\blocked(\ceu{p';p''},e\alpha,n)=\blocked(\ceu{p'},e\alpha,n)$.
    \begin{case}
    \item$p'=\ceu\Skip$.  Then we can derive~$\delta$ using the
      axiom~\eqref{def:orig:inner:seq-skip}, and as such, 
      $\delta=\<p'',e\alpha,m>$ and according to
      definition~\ref{def:orig:blocked},
      $\blocked(\ceu\Skip,e\alpha,n)=0$.
    \item$p'=\ceu{\Attr{v}{a}}$, for some~$v\in{V}$ and~$a\in{A}$.
      Then we can derive~$\delta$ using the
      axiom~\eqref{def:orig:inner:seq-attr}, and as such,
      $\delta=\<\ceu{p''},e\alpha,m'>$, with $m'=m[v/\eval(a)]$, and 
      according to definition~\ref{def:orig:blocked},
      $\blocked(\ceu{\Attr{v}{a}},e\alpha,n)=0$.
    \item$p'=\ceu{\Break}$. Then we can derive~$\delta$ using the
      axiom~\eqref{def:orig:inner:seq-break}, and as such,
      $\delta=\<\ceu\Break,e\alpha,m>$, and 
      according to definition~\ref{def:orig:blocked},
